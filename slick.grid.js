 ...
 *
 * NOTES:
 * 		ADDED ACCESSIBLITIY CODE CHANGE IN V 2.2 CHANGE MADE BY Sanjeev Sinha
 *  * 
...
(function ($) {
    ...
    function SlickGrid(container, data, columns, options) {
        ...
        var viewportH, viewportW,viewportScrollH;
        ...
        function init() {
            ...
            $container
                ...
                .css("outline", 0)
                .addClass(uid)
         // .attr({'role': 'grid', 'aria-rowcount' : data.getLength()+1})
                .attr("role","grid")
                .addClass("ui-widget");
            ...
            $focusSink = $("<div  role='presentation' hideFocus style='position:fixed;width:0;height:0;top:0;left:0;outline:0;'></div>").appendTo($container);

            // Containers used for scrolling frozen columns and rows
            $paneHeaderL = $("<div class='slick-pane slick-pane-header slick-pane-left' role='presentation'   />").appendTo($container);
            $paneHeaderR = $("<div  class='slick-pane slick-pane-header slick-pane-right' id='"+options.elemid+"_headerpaneright"+"' role='presentation'  />").appendTo($container);
            $paneTopL = $("<div class='slick-pane slick-pane-top slick-pane-left' role='presentation'  />").appendTo($container);
            $paneTopR = $("<div class='slick-pane slick-pane-top slick-pane-right' id='"+options.elemid+"_panetopright"+"' role='presentation'  />").appendTo($container);
            $paneBottomL = $("<div role='presentation' class='slick-pane slick-pane-bottom slick-pane-left'  />").appendTo($container);
            $paneBottomR = $("<div role='presentation' class='slick-pane slick-pane-bottom slick-pane-right' id='"+options.elemid+"_panebottomright"+"'  />").appendTo($container);

            // Append the header scroller containers
            $headerScrollerL = $("<div class='ui-state-default slick-header slick-header-left' role='presentation' />").appendTo($paneHeaderL);
            $headerScrollerR = $("<div class='ui-state-default slick-header  slick-header-right' role='presentation' />").appendTo($paneHeaderR);

            // Cache the header scroller containers
            $headerScroller = $().add($headerScrollerL).add($headerScrollerR);

            //aria-owns for header
            var headerAriaOwns="";
                for(var c = 0, cc = columns.length; c < cc; c++){
                headerAriaOwns += uid + columns[c].ID + c + " ";
            }
            // Append the columnn containers to the headers
            $headerL = $("<div class='slick-header-columns slick-header-columns-left' role='row' aria-owns='" +headerAriaOwns+ "' style='left:-1000px' />").appendTo($headerScrollerL);
            $headerR = $("<div class='slick-header-columns slick-header-columns-right' role='presentation' style='left:-1000px'  />").appendTo($headerScrollerR);

            // Cache the header columns
            $headers = $().add($headerL).add($headerR);

            $headerRowScrollerL = $("<div class='ui-state-default slick-headerrow' role='presentation' />").appendTo($paneTopL);
            $headerRowScrollerR = $("<div class='ui-state-default slick-headerrow' role='presentation' />").appendTo($paneTopR);

            $headerRowScroller = $().add($headerRowScrollerL).add($headerRowScrollerR);

            $headerRowSpacerL = $("<div style='display:block;height:1px;position:absolute;top:0;left:0;' role='presentation'></div>")
                .css("width", getCanvasWidth() + scrollbarDimensions.width + "px")
                .appendTo($headerRowScrollerL);
            $headerRowSpacerR = $("<div style='display:block;height:1px;position:absolute;top:0;left:0;' role='presentation'></div>")
                .css("width", getCanvasWidth() + scrollbarDimensions.width + "px")
                .appendTo($headerRowScrollerR);


            $headerRowL = $("<div class='slick-headerrow-columns slick-headerrow-columns-left' role='presentation' />").appendTo($headerRowScrollerL);
            $headerRowR = $("<div class='slick-headerrow-columns slick-headerrow-columns-right'  role='presentation' />").appendTo($headerRowScrollerR);

            $headerRow = $().add($headerRowL).add($headerRowR);

            // Append the top panel scroller
            $topPanelScrollerL = $("<div class='ui-state-default slick-top-panel-scroller' role='presentation'/>").appendTo($paneTopL);
            $topPanelScrollerR = $("<div class='ui-state-default slick-top-panel-scroller' role='presentation' />").appendTo($paneTopR);

            $topPanelScroller = $().add($topPanelScrollerL).add($topPanelScrollerR);

            // Append the top panel
            $topPanelL = $("<div class='slick-top-panel' style='width:10000px' role='presentation' />").appendTo($topPanelScrollerL);
            $topPanelR = $("<div class='slick-top-panel' style='width:10000px' role='presentation' />").appendTo($topPanelScrollerR);
            ...
            $viewportTopL = $("<div  class='slick-viewport slick-viewport-top slick-viewport-left'   hideFocus role='presentation'/>").appendTo($paneTopL);
            $viewportTopR = $("<div  class='slick-viewport slick-viewport-top slick-viewport-right'   hideFocus role='presentation'/>").appendTo($paneTopR);
            $viewportBottomL = $("<div  role='presentation' class='slick-viewport slick-viewport-bottom slick-viewport-left'    hideFocus />").appendTo($paneBottomL);
            $viewportBottomR = $("<div  role='presentation' class='slick-viewport slick-viewport-bottom slick-viewport-right'  hideFocus />").appendTo($paneBottomR);
            ...
            $canvasTopL = $("<div class='grid-canvas grid-canvas-top grid-canvas-left'   hideFocus role='presentation' />").appendTo($viewportTopL);
            $canvasTopR = $("<div class='grid-canvas grid-canvas-top grid-canvas-right'  hideFocus role='presentation' />").appendTo($viewportTopR);
            $canvasBottomL = $("<div role='presentation' class='grid-canvas grid-canvas-bottom grid-canvas-left'   hideFocus />").appendTo($viewportBottomL);
            $canvasBottomR = $("<div role='presentation' class='grid-canvas grid-canvas-bottom grid-canvas-right'  hideFocus />").appendTo($viewportBottomR);
        ...
        function finishInitialization() {
            if (!initialized) {
                ...
                // $viewport 
                 ...
                 if(hasFrozenRows)
                 {
                 $viewportBottomL.bind("scroll mousewheel", divScrollR);
                 $viewportBottomR.bind("scroll mousewheel", divScrollL);
                 //$viewportTopR.bind("scroll mousewheel", scrollTopRightPane);
                 //$headerScrollerR.bind("scroll mousewheel", scrollTopRHeader);
                 }
                else
                 {
                  $viewportTopL.bind("scroll mousewheel", divScrollR);
                  $viewportTopR.bind("scroll mousewheel", divScrollL);
                  //$headerScrollerR.bind("scroll mousewheel", scrollTopRHeader);
                 }
         ...
         function divScrollR()
        {
            if(hasFrozenRows)
            {
             var Right =  $viewportBottomL[0];                   
             var Left= $viewportBottomR[0];            
             Left.scrollTop = Right.scrollTop;
            }
            else
            {
             var Right = $viewportTopL[0];                   
             var Left= $viewportTopR[0];            
             Left.scrollTop = Right.scrollTop;
            }
        }
        function divScrollL()
    ...
    var maxScrollDistanceX = $viewportScrollContainerY[0].scrollWidth - $viewportScrollContainerY[0].clientWidth;
         ...
         var hScrollDist = Math.abs(scrollLeft - prevScrollLeft);
            if (hScrollDist) 
            {
                ...
                var offsetLeft=$($viewportScrollContainerX[0].firstChild).offset();
					$($headerScrollContainer[0].firstChild).offset({left:-1000+offsetLeft.left});
				 ...
				 if (options.frozenColumn > -1)
				  {
					...
					//$viewportTopR[0].scrollLeft = scrollLeft;
                      $($viewportTopR[0].firstChild).offset({left:offsetLeft.left});
                    }
                  } 
    ...
    function scrollTopRHeader()
    {
            if(hasFrozenRows)
            {
             var headerRightPane = $('#paneHeaderTopR').get(0);
             var topRightPane= $('#viewpTopR').get(0);
             var bottomRight= $('#viewpBottomR').get(0);
             bottomRight.scrollLeft = headerRightPane.scrollLeft;
             topRightPane.scrollLeft = headerRightPane.scrollLeft;
            }
            else
            {
              var headerRightPane = $('#paneHeaderTopR').get(0);
   ...
   function scrollTopRightPane()
     {
            if(hasFrozenRows)
            {
             ...
             var topRightPane= $('#viewpTopR').get(0);
             var bottomRight= $('#viewpBottomR').get(0);
             bottomRight.scrollLeft = topRightPane.scrollLeft;
             headerRightPane.scrollLeft =topRightPane.scrollLeft;
            }
            else
            {
              var headerRightPane = $('#paneHeaderTopR').get(0);
        ...
        function measureScrollbar() {
            var $c = $("<div style='position:absolute; top:-10000px; left:-10000px; width:100px; height:100px; overflow:scroll;' role='presentation'></div>").appendTo("body");
            var dim = {
        ...
        function updateCanvasWidth(forceColumnWidthsUpdate) {
            ...
            var widthChanged;
            var viewContainerWidth= parseFloat($.css($container[0], "width", true));
            ...
            if (widthChanged || options.frozenColumn > -1 || hasFrozenRows) {
                ...
                if (options.frozenColumn > -1) {
                   ...
                   // $viewportTopR.width(viewportW - canvasWidthL);
                    if((viewportW - canvasWidthL) <(viewContainerWidth - canvasWidthL))
                        {
                        $paneHeaderR.css('width', viewportW - canvasWidthL);
                        $viewportTopR.width(viewportW - canvasWidthL);
                        // $headerR.width(viewportW - canvasWidthL);
                        }
                        else
                        {
                        $paneHeaderR.css('width', viewContainerWidth - canvasWidthL);
                        $viewportTopR.width(viewContainerWidth - canvasWidthL);
                       // $headerR.width(viewContainerWidth - canvasWidthL);
                        }
                    ...
                    if (hasFrozenRows) {
                        ...
                        {
                        $viewportBottomR.width(viewContainerWidth - canvasWidthL);
                        }
        ...
        function createColumnHeaders() {
            ...
            for (var i = 0; i < columns.length; i++) {
                ...
                var header = $("<div class='ui-state-default slick-header-column' id='" +uid + m.ID +i+ "' role='columnheader' />")
        ...
        function measureCellPaddingAndBorder() {
            ...
            el = $("<div class='ui-state-default slick-header-column' style='visibility:hidden' role='presentation'>-</div>").appendTo($headers);
        ...
        function appendRowHtml(stringArrayL, stringArrayR, row, range, dataLength) {
            ...
            var frozenRowOffset = getFrozenRowOffset(row);
            		...
            		rowid=uid+'_'+row;
           	oldrow=row-1
           		oldrowid=uid+'_'+oldrow;
         
       // $('#sgrowL'+oldrowid).attr("aria-owns",ariaown);
         
         /* sanjeev chnages for aria-owns attribute values */
         var ariaOwns = "";
         for(var c = 0, cc = columns.length; c < cc; c++){
            ariaOwns += uid+"_SGr"+row+"c"+c +" ";
         }
         /* aria owns attribute value calculation done */
            var rowLHtml = "<div id='sgrowL"+rowid+"' class='ui-widget-content " + rowCss + "'aria-owns='"+ariaOwns+"' role='row' style='top:"
                + (getRowTop(row) - frozenRowOffset )
                + "px'>";
            var rowRHtml = "<div id='sgrowR"+rowid+"' class='ui-widget-content " + rowCss + "' style='top:"
            + (getRowTop(row) - frozenRowOffset )
            + "px'>";
            stringArrayL.push(rowLHtml);

            if (options.frozenColumn > -1) {
                stringArrayR.push(rowRHtml);
            }
            ...
            for (var i = 0, ii = columns.length; i < ii; i++) {
                ...
                if (columnPosRight[Math.min(ii - 1, i + colspan - 1)] > range.leftPx) {
                    ...
                    appendCellHtml(stringArrayL, row, i, colspan, d);
                    /* d parameter was missing here and has been added to resolve a problem
                     * where we were adding multiple column dynamically and frozen column cell
                     * was not getting rendered 
                     * ref :-https://github.com/mleibman/SlickGrid/issues/1002
                     */
        ...
        function appendCellHtml(stringArray, row, cell, colspan, item) {
            ...
            var gridcellRole = cellCss.indexOf('rowheader') === -1 ?'gridcell':'rowheader';

        /*stringArray.push("<div class='" + cellCss +
                       "' aria-describedby='" +uid + m.id + cell+
                       "' tabindex='-1' role='"+gridcellRole+"' id='"+uid+"_SGr"+row+"c"+cell+"'>");*/
                       stringArray.push("<div class='" + cellCss +
                       "'   role='"+gridcellRole+"' id='"+uid+"_SGr"+row+"c"+cell+"'>");
     	rowid=uid+'_'+row;
     	
     	var colid='SGr'+row+'c'+cell;
        ...
        function getViewportHeight() {
            if (options.autoHeight) {
             ...
             viewportScrollH= parseFloat($.css($container[0], "height", true))
                    - parseFloat($.css($container[0], "paddingTop", true))
                    - parseFloat($.css($container[0], "paddingBottom", true))
                    - parseFloat($.css($headerScroller[0], "height"))
                    - topPanelH
        ...
        function getViewportWidth() {
            ...
            for (var i = 0, ii = columns.length; i < ii; i++) {
             ...
             tempWidth += scrollbarDimensions.width;
             viewportW= tempWidth;            
        }
        ...
        function resizeCanvas() {
            ...
            if (options.frozenColumn > -1) {
                ...
                if (hasFrozenRows) {
                    $paneBottomL.css({
                        'top': paneBottomTop, 'height': viewportScrollH
                    });
                    ...
                    $viewportBottomR.height(viewportScrollH  - paneTopH);
                }
            } else {
                if (hasFrozenRows) {
                    $paneBottomL.css({
            ...
            if (hasFrozenRows) {
                $viewportBottomL.height(viewportScrollH - paneTopH);
                ...
                $viewportTopR.height(viewportScrollH);
                 $viewportTopL.height(viewportScrollH);
            }
        ...
        function _handleScroll(isMouseWheel) {
            ...
            if (vScrollDist) {
                ...
                if (options.frozenColumn > -1) {
                    if (hasFrozenRows && !options.frozenBottom) {
                          // $viewportBottomL[0].scrollTop = scrollLo;
                     var off=$($viewportScrollContainerY[0].firstChild).offset();
                     $($viewportBottomL[0].firstChild).offset({top:off.top});
                    } else {
